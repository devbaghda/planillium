...
- The Diary could appear to start the day whenever you first touched the PC that morning
  ...
  permanently instead of showing up as unaccounted time. It's now logged the moment the gap is
  detected, the same as it always used to be — answering the check-in (if you do) just fills in
  the real description instead of leaving it blank, rather than the two ever creating duplicate
  entries.
